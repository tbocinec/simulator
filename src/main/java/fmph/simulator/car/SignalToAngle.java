...
public class SignalToAngle {
    ...
    public double transformToAngle(double signal) {
        ...
        double maxAngle = ContextBuilder.getContext().getCarManagement().getModelManagment().getActual().getMaxWheelAngle();
        ...
        if(ifProbably(ContextBuilder.getContext().getNoiseController().getChangePWM())){
            double rand  = new Random().nextInt(50) +10;
            angle = (angle/100) * rand;
            new Message("[Noise] Attenuation PWM " +rand, MessageType.INFO);
        }
        return  angle;
    }

    private double useSimpleLinear(double signal){
        return -(0.035 * signal+4);
    }
